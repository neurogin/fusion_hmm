function prune_iclabel_components_and_export_metadata(root_raw_eeglab, out_base_dir, qc_table_dir, varargin)
%PRUNE_ICLABEL_COMPONENTS_AND_EXPORT_METADATA Stage-1 ICLabel pruning helper.
%
% What this helper does:
%   Loads author-preprocessed EEGLAB `.set` files, applies the preserved
%   manuscript ICLabel pruning rule, and writes both:
...
%   - `out_base_dir`: Stage-1 output root containing the with-ICA and clean
%     subfolders
%   - `qc_table_dir`: folder for run-level ICLabel summary tables
%
% Key outputs:
%   - `<out_base_dir>\<withICA_subdir>\<stem>_desc-<TAG>_withICA.set`
%   - `<out_base_dir>\<clean_subdir>\<stem>_desc-<TAG>_clean.set`
%   - `eeg_ic_prune_summary_<TAG>.csv`
%   - `eeg_ic_prune_lists_<TAG>.csv`
%   - optional per-run `*_iclabel_table_<TAG>.csv`
%
% Important note:
%   The scientific pruning behavior is preserved exactly from the original
%   Stage-1 implementation. This promotion pass improves readability and
%   naming only; it does not change thresholds, policies, or output names.
