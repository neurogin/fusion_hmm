function out_tsv = merge_exclusion_union_masks(in_tsv, out_tsv, varargin)
%MERGE_EXCLUSION_UNION_MASKS Merge one run's exclusion intervals into a union.
%
% What this helper does:
%   Reads one Stage-1 exclusion TSV, keeps the manuscript-facing exclusion
%   labels, merges overlapping or touching windows, and writes a
%   run-specific union mask TSV.
%
% When it is used:
%   Called by `batch_merge_exclusion_union_masks.m`.
%
% Key inputs:
%   - `in_tsv`: one `*_bst_exclusions.tsv` file
%   - `out_tsv`: destination `*_excl_union.tsv`
%
% Key outputs:
%   - merged union TSV at `out_tsv`
%   - optional per-run QC CSV sidecar beside `out_tsv`
%
% Important note:
%   The interval-merging behavior is preserved exactly. This helper changes
%   naming and documentation only.
