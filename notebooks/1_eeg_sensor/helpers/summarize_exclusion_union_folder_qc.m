function summarize_exclusion_union_folder_qc(union_dir, out_dir, varargin)
%SUMMARIZE_EXCLUSION_UNION_FOLDER_QC Build folder-level exclusion QC summaries.
%
% What this helper does:
%   Scans the Stage-1 `*_excl_union.tsv` files for one folder, computes
%   run-level exclusion summary metrics, and flags common problems that
%   matter for downstream QC and alignment.
%
% When it is used:
%   Called by `summarize_exclusion_union_qc.m`.
%
% Key inputs:
%   - `union_dir`: folder containing `*_excl_union.tsv`
%   - `out_dir`: folder for the summary CSV
%
% Key outputs:
%   Writes `excl_union_qc_summary.csv`
%
% Important note:
%   The run-duration lookup from Brainstorm raw-link MAT files is preserved
%   exactly from the original helper.
